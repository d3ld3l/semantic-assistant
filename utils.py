...
# Построение обратного отображения для синонимов
NORMALIZED_SYNONYMS = {}
...
# Семантический поиск с новой моделью
def semantic_search(query, df, top_k=5, threshold=0.5):
    query_proc = preprocess(query)

    # E5 требует специальной разметки
    query_emb = model.encode("query: " + query_proc, convert_to_tensor=True)
    phrase_texts = ["passage: " + p for p in df['phrase_proc']]
    phrase_embs = model.encode(phrase_texts, convert_to_tensor=True)
